# Маркбэйс id — Войти через Маркбэйс id

> **Версия:** 3.0.0 | **Slug:** `auth-widget` | **Категория:** Auth / SSO

Полная документация для интеграции авторизации **Маркбэйс id** в ваш проект.
...
| **[Маркбэйс id_LOGIN_AS.md](../Маркбэйс id_LOGIN_AS.md)** | Подключение как модуль: проект с **собственной регистрацией** — одна кнопка «Войти как», передача данных (cookie / wsid_code) |
| **[Маркбэйс id.css](./Маркбэйс id.css)** | Ready-to-use CSS для кнопки и формы (CDN / npm) |
...
**Маркбэйс id** — единый провайдер аутентификации платформы MarkBase.
Любой сервис (внутренний модуль или сторонний сайт) может добавить кнопку **«Войти через Маркбэйс id»** и получить доступ к данным пользователя через стандартный OAuth 2.0 + OIDC flow.

### 1.1 Как это работает

```
   Ваш сервис (waygpt.ru)              Маркбэйс id (auth.markbase.ru)
┌─────────────────────────┐         ┌─────────────────────────────┐
│                         │         │                             │
│  [Войти через Маркбэйс id] │ ──────► │  У пользователя есть        │
│                         │         │  аккаунт?                   │
│  ───── или ─────        │         │                             │
│                         │         │  ДА → Account Picker /      │
...
- **Вам не нужно** реализовывать регистрацию, подтверждение email, captcha — Маркбэйс id делает всё сам

---

## 2. Кнопка «Войти через Маркбэйс id»
...
```html
<!-- Primary, размер md -->
<button class="wsid-btn wsid-btn--primary wsid-btn--md" onclick="Маркбэйс id.login()">
  ...
  Войти через Маркбэйс id
</button>
```

### 2.4 CSS

Полные стили — в файле [Маркбэйс id.css](./Маркбэйс id.css).
...
```javascript
// Popup-режим
const result = await Маркбэйс id.login({
  mode: 'popup',
  prompt: 'select_account',
});
...
```javascript
Маркбэйс id.on('login', (user) => console.log('Logged in:', user.email));
Маркбэйс id.on('logout', () => console.log('Logged out'));
Маркбэйс id.on('token_expired', () => Маркбэйс id.refreshToken());
Маркбэйс id.on('error', (err) => console.error(err.code, err.message));
```

### 8.6 Выход

```javascript
await Маркбэйс id.logout();
await Маркбэйс id.logout({ global: true }); // из всех приложений
```
...
<script>
  ...
  Маркбэйс id.renderButton('#wsid-button', { variant: 'primary', size: 'lg', width: '100%' });
</script>
```
...
```jsx
import { Маркбэйс idButton, Маркбэйс idProvider } from '@markbase/Маркбэйс id-sdk/react';

function App() {
  return (
    <Маркбэйс idProvider clientId="YOUR_CLIENT_ID" redirectUri="/callback" scope="openid profile email">
      <LoginPage />
    </Маркбэйс idProvider>
  );
}
...
Когда пользователь нажимает «Войти через Маркбэйс id» и у него **нет аккаунта**, Маркбэйс id автоматически проводит его через регистрацию:
...
| Хранить пароли Маркбэйс id | Проверить `email_verified: true` |
...
- Кнопка ДОЛЖНА использовать стандартную иконку Маркбэйс id (globe SVG)
- ЗАПРЕЩЕНО изменять текст, цвета или пропорции иконки
- Допустимые тексты: «Войти через Маркбэйс id», «Продолжить с Маркбэйс id», «Маркбэйс id»
...
Маркбэйс id.renderButton('#login-btn', {
  variant: 'primary',
  size: 'lg',
  width: '100%',
});

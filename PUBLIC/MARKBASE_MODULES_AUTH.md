...
Во **всех** модулях и проектах на доменах `*.markbase.ru` вход и регистрация выполняются **только через Марбэйс id (UAM)** на auth.markbase.ru. Собственных форм ввода email/пароля и капчи на стороне модуля **не должно быть**.

- **Вход / регистрация:** кнопка **«Единый аккаунт»** + подпись **«Марбэйс id»** и иконка пояснения → **всплывающее окно** (popup) с auth.markbase.ru/login или /register.
...
| Требование | Описание |
|------------|----------|
| Нет своих форм входа/регистрации | Только кнопка «Единый аккаунт / Марбэйс id» |
| Вход через popup | auth.markbase.ru в popup с `return_url` |
| Callback | `/auth/callback?to=...` + `postMessage` |
| Один аккаунт | Марбэйс id (UAM); cookie `uam_session` на `.markbase.ru` |

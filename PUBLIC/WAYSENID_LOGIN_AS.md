# «Войти через Маркбэйс id» — подключение как модуль (аналог «Войти через Google / Яндекс»)

Документ описывает, как добавить кнопку **«Войти через Маркбэйс id»** в проект с **собственной системой регистрации** — так же, как подключают «Войти через Google» или «Войти через Яндекс ID». Один раз настроили — данные пользователя передаются корректно, без дублирования логики входа и регистрации на вашей стороне.
...
- **«Войти через Маркбэйс id»** — ещё один способ входа: пользователь переходит на auth.markbase.ru, входит или регистрируется там (включая капчу и подтверждение email), после чего возвращается к вам с **идентификатором и данными пользователя**.
- Вам **не нужно** реализовывать формы регистрации/логина для Маркбэйс id, хранить пароли или показывать капчу — всё это делает auth.markbase.ru.

---

## 2. Какой способ авторизации выбрать (дерево решений)

В экосистеме три способа подключить вход Маркбэйс id. Выбирайте **один** по таблице — не смешивайте протоколы.
...
│  [ Войти через Маркбэйс id ]   ← одна кнопка                    │
└──────────────────────────────┬──────────────────────────────┘
                                │
                                ▼  Редирект
...
│  Капча, подтверждение email — всё на стороне Маркбэйс id         │
└──────────────────────────────┬──────────────────────────────┘
                                │
        ┌───────────────────────┴───────────────────────┐
...
- Текст: **«Войти через Маркбэйс id»** (или «Продолжить с Маркбэйс id»).
...
- Стили и размеры — см. [auth-widget/README.md](./auth-widget/README.md) и [auth-widget/Маркбэйс id.css](./auth-widget/Маркбэйс id.css).
...
| Шаг | Действие |
|-----|----------|
| 1 | Добавить на страницу входа одну кнопку «Войти через Маркбэйс id» (редирект на auth.markbase.ru с `return_url`). |
...
| 4 | Не дублировать: формы логина/регистрации Маркбэйс id, капчу и хранение паролей Маркбэйс id у себя. |
| 5 | Использовать полученные поля `user_id`, `email`, `display_name` и при необходимости `metadata` как единственный источник правды о пользователе Маркбэйс id. |
...
При такой схеме кнопка «Войти через Маркбэйс id» подключается как отдельный модуль (как «Войти через Google»), все данные передаются через официальные эндпоинты UAM, без дублей логики и без лишней нагрузки на вашу систему.
